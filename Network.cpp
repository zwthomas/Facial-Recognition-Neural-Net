...
Network::Network(int numLayers, int numNodes[], int numInputs) {
    ...
    LAYERS = new sigmoid*[numLayers];
    for (int layerCount = 0; layerCount < numLayers; layerCount++) {                // Creates each layer
        LAYERS[layerCount] = new sigmoid(numNodes[layerCount]);

        for (int nodeCount = 0; nodeCount < numNodes[layerCount]; nodeCount++) {    // Creates each node in the layer
            if (layerCount == 0) {
                LAYERS[layerCount][nodeCount] = sigmoid(numInputs + 1);               // Number of inputs in first layer
                // Depends on number of inputs
            } else {
                LAYERS[layerCount][nodeCount] = sigmoid(numNodes[layerCount - 1] + 1); // Rest of rows depend on the previous
            }
        }
    }
}
